...
block_manager::alloc_block()
{
   ...
   */
  //first data block
  blockid_t fir_data_blockid=IBLOCK(sb.ninodes,sb.nblocks);
  //first block bit map block
  blockid_t fir_bitmap_blockid=BBLOCK(fir_data_blockid);
  //last block bit map block
  blockid_t fin_bitmap_blockid=BBLOCK(sb.nblocks);
  ...
  uint32_t block_bitmap_index=fir_data_blockid%BPB;

  for(blockid_t i=fir_bitmap_blockid;i<fin_bitmap_blockid;i++)
  {
    read_block((uint32_t)i,per_block_bitmap);
    //judge whether block is free by bit in bitmap
    for(int j=block_bitmap_index;j<BPB;j++)
    {
      //index in char
      int byte_index=j/8;
      //bit in byte
      ...
      //get the target bit
      bool is_free=(target_byte>>(7-bit_index))&0x1;

      //the block has been alloced,judge the next
      if(is_free==true)
      {
        continue;
      }
      //the block is free,alloc it
      else
      {
        //switch the target bit to 1
        target_byte=target_byte|(0x1 << (7-bit_index));
        per_block_bitmap[byte_index]=target_byte;
        write_block(i,per_block_bitmap);

        delete[] per_block_bitmap;
        blockid_t alloc_blockid=j+(i-2)*BPB;

        return alloc_blockid;
      }
    }
