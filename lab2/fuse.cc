...
//create link
void
fuseserver_createlink(fuse_req_t req, const char*target_name, fuse_ino_t parent, const char*link_name)
{
    struct fuse_entry_param e;
    // In yfs, timeouts are always set to 0.0, and generations are always set to 0
    e.attr_timeout = 0.0;
    ...
    yfs_client::inum inode_id;
    //create link
    ret=yfs->createlink(parent,link_name,target_name,inode_id);
    if(ret==yfs_client::OK)
    {
        e.ino=inode_id;
        getattr(inode_id,e.attr);
        fuse_reply_entry(req, &e);
    }
    else
    {
...
fuseserver_readlink(fuse_req_t req, fuse_ino_t ino)
{
    ...
    yfs_client::status ret;
    //create a string to store the read link
    std::string link_content="";
    ret=yfs->readlink(ino,link_content);
    if(ret==yfs_client::OK)
    {
        fuse_reply_readlink(req, link_content.data());
    }
    else
    {
        if(ret==yfs_client::NOENT)
...
main(int argc, char *argv[])
{
     ...
     * rmdir, etc.
     * */
    fuseserver_oper.symlink = fuseserver_createlink;
